...
class ShrecPartialDataset(Dataset):
    def __init__(self, root_dir, name="cuts", k_eig=128, n_fmap=30, use_cache=True, op_cache_dir=None, use_adj=False, verbose=True):
        ...
        for x, y in map(lambda z: (z[:z.rfind("_")], z[z.rfind("_") + 1:]), all_combs):
            print(x,y)
...
class Tosca2(Dataset):
    """
    Download dataset from https://vision.in.tum.de/data/datasets/partial
    """
    def __init__(self, path, name="cuts", selected=False, use_adj=True, k_eig=128, n_fmap=30, n_samples=None, op_cache_dir=None, use_cache=True, verbose=True):
        super().__init__(path, use_adj=use_adj)
    ...
    def __getitem__(self, idx):
        ...
        evec_1, evec_2, mass2 = shape1["evecs"][:, :self.n_fmap], shape2["evecs"][:, :self.n_fmap], shape1["mass"]
        trans_evec1 = evec_1.t() @ torch.diag(mass1)

        P = torch.zeros(evec_1.size(0), evec_2.size(0))
        P[range(evec_2.size(0)), map12.flatten()] = 1
        C_gt = trans_evec1 @ P @ evec_2
        ...
        return {"shape1": shape2, "shape2": shape1, "C_gt": C_gt,
                "map21": map21, "gt_partiality_mask12": gt_partiality_mask12, "gt_partiality_mask21": gt_partiality_mask21}    
