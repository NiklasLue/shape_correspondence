...
def FM_batch_eval(batch_data, net, shape1, shape2):
    ...
    log_obj = (name1, name2, C_pred.detach().cpu().squeeze(0), C_gt.detach().cpu().squeeze(0),
                            overlap_score12.detach().cpu().squeeze(0), overlap_score21.detach().cpu().squeeze(0),
...
def eval_net(cfg, model_path, predictions_name, return_pred_p2p=False, return_dist=False, mode="FM", n_samples=None):
    ...
    for i, data in enumerate(tqdm.tqdm(test_loader)):
        ...
        # do iteration
        if mode=="FM":
            p2p_pred, overlap_score12, overlap_score21, use_feat1, use_feat2, log_obj = FM_batch_eval(data, dpfm_net, shape1, shape2)
        elif mode=="CFM":
        ...
        if return_pred_p2p:
            pred_p2p_list.append({'mesh1': shape1["mesh"], 'mesh2': shape2["mesh"], 'p2p': p2p_pred})
#'p2p': p2p_pred,
